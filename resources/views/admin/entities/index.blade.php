...
<div id="entities-list">
    <div class="row">
        <x-admin.table-card title="Entities" :count="$entities->count()" 
            data-row-reorder="true" 
            data-row-reorder-url="{{ route('admin.entities.reorder') }}">
            <x-slot:actions>
                <x-admin.live-search-bar :new-route="route('admin.entities.create')" />
            </x-slot:actions>
            <x-slot:head>
                <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Localização</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-center">Website</th>
            ...
            <tr data-edit-url="{{ route('admin.entities.edit', $entity) }}" data-id="{{ $entity->id }}" style="cursor: pointer;">
                <!-- mostrar apenas ícone (não mostrar o número da ordem) mas manter data-ordem para RowReorder -->
                <td class="text-xs text-center reorder-handle" data-ordem="{{ $entity->ordem }}" style="cursor:move;">
                    <i class="fas fa-bars"></i>
                    {{-- número de ordem escondido intencionalmente --}}
                </td>
                <td class="text-xs fw-bold text-center" data-order="{{ $entity->id }}" data-id="{{ $entity->id }}">#{{ $entity->id }}</td>
                <td class="text-xs fw-bold">{{ $entity->name }}</td>
                <td class="text-xs">{{ $entity->description }}</td>
                <td class="text-xs">{{ $entity->location }}</td>
                <td class="text-xs">{{ $entity->website }}</td>
                <td class="text-xs">
                    <div class="d-flex justify-content-center align-items-center" style="gap:.75rem;">
                        <a href="{{ route('admin.entities.edit', $entity) }}" class="text-xs font-weight-bold"
                            style="color:#5e72e4;" title="Editar">Editar</a>
                        <form action="{{ route('admin.entities.destroy', $entity) }}" method="POST"
                            class="m-0 p-0 js-delete-confirm" data-confirm="Eliminar este entity?" data-check-relations="true" data-entity="entities" data-id="{{ $entity->id }}">
                            @csrf
                            @method('DELETE')
            ...
            @empty
            <tr>
                <td colspan="7" class="text-center text-muted py-4">Nenhum entity encontrado.</td>
            </tr>
            @endforelse
        </x-admin.table-card>
    </div>
